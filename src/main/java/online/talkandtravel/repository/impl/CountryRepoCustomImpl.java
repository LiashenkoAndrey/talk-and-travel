...
public class CountryRepoCustomImpl implements CountryRepoCustom {
  ...
  private CountryDtoWithParticipantsAmountAndMessages getCountryDto(String countryName) {
    return em.createQuery(
            "select c.id as id , c.name as name, c.flagCode as flagCode from Country c where c.name = :name",
            CountryDtoWithParticipantsAmountAndMessages.class)
  ...
  private Long getParticipantsAmount(Long countryId) {
    return (Long)
        em.createNativeQuery(
                "select count(*) from public.participant_countries pc where pc.country_id = :country_id")
