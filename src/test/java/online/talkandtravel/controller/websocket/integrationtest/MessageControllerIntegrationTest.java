...
public class MessageControllerIntegrationTest extends StompIntegrationTest {
  ...
  private void assertMessage(int index, MessageType messageType, String content,
      Long chatId) {
    ...
    assertThat(messageDto.creationDate()).isNotNull();
    assertThat(messageDto.user().id()).isEqualTo(getAlice().getId());
  }

  private static Stream<Arguments> testUserSendsMessageSuccessfullyArgs() {
    return Stream.of(
        Arguments.of(new SendMessageRequest("hello!", ARUBA_CHAT_ID, null), 0, MessageType.TEXT,
            "hello!", ARUBA_CHAT_ID),
        Arguments.of(new SendMessageRequest("how are you??", ARUBA_CHAT_ID, null), 1,
            MessageType.TEXT, "how are you??", ARUBA_CHAT_ID),
        Arguments.of(new SendMessageRequest("new message1", ANGOLA_CHAT_ID, null), 2,
            MessageType.TEXT, "new message1", ANGOLA_CHAT_ID),
        Arguments.of(new SendMessageRequest("new message2", ANGOLA_CHAT_ID, null), 3,
            MessageType.TEXT, "new message2", ANGOLA_CHAT_ID),
        Arguments.of(new SendMessageRequest("new message3", ANGOLA_CHAT_ID, null), 4,
            MessageType.TEXT, "new message3", ANGOLA_CHAT_ID)
    );
  }
